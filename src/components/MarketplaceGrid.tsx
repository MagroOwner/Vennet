...
const gridClasses: Record<DisplaySize, string> = {
  small: "grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5",
  medium: "grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",
    ...
    <>
      <div className="surface-card p-4 sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><h2 className="text-2xl font-black tracking-[-.04em] text-[#10151f]">{heading}</h2><p className="mt-1 text-sm text-slate-600">{visibleListings.length} of {listings.length} offers matched</p></div>
          <div className="inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1" aria-label="Listing display size">
            {(["small", "medium", "large"] as DisplaySize[]).map((option) => <button key={option} type="button" onClick={() => chooseSize(option)} className={size === option ? "rounded-lg bg-[#10151f] px-3 py-1.5 text-sm font-bold capitalize text-white" : "rounded-lg px-3 py-1.5 text-sm font-semibold capitalize text-slate-600 transition hover:bg-white"}>{option}</button>)}
          </div>
        </div>
        <div className="mt-5 grid gap-3 lg:grid-cols-[1.4fr_repeat(4,minmax(0,.7fr))]">
          ...
          <input id="marketplace-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search products, skills, tools, and creators" className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-[#10151f] placeholder:text-slate-500 outline-none ring-emerald-300 transition focus:border-emerald-400 focus:ring-2" />
