...
export function SellerSidebar() {
  ...
  return <>
    ...
    <aside aria-label="Seller tools" className={"fixed bottom-0 left-0 top-0 z-50 w-[20rem] overflow-y-auto border-r border-emerald-100 bg-gradient-to-b from-emerald-50 via-white to-slate-100 p-5 shadow-2xl shadow-slate-950/25 transition-transform duration-300 " + (open ? "translate-x-0" : "-translate-x-full")}>
      ...
      <nav className="mt-5 space-y-2">{tools.map(([label, href, detail]) => <Link key={label} href={href} onClick={() => setOpen(false)} className="group block rounded-xl border border-slate-200 bg-white px-3 py-3 shadow-sm transition hover:border-emerald-300 hover:bg-emerald-50"><p className="font-black text-slate-800 group-hover:text-emerald-900">{label}</p><p className="mt-0.5 text-xs font-medium text-slate-500">{detail}</p></Link>)}</nav>
