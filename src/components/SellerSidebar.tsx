...
export function SellerSidebar() {
  ...
  return <>
    ...
    {open && <button aria-label="Close seller tools overlay" type="button" onClick={close} className="fixed inset-0 z-40 cursor-default bg-slate-950/45 backdrop-blur-sm" />}
    <aside aria-label="Seller tools" className={"fixed inset-y-0 left-0 z-50 overflow-y-auto border-r border-emerald-100 bg-gradient-to-b from-emerald-50 via-white to-slate-100 shadow-2xl shadow-slate-950/35 transition-[width,transform] duration-300 " + (open ? "translate-x-0 " : "-translate-x-full ") + (minimized ? "w-20 p-3" : "w-[20rem] p-5")}>
