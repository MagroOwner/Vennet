...
import { getWelcomeResponse } from "@/lib/welcome";
...
export async function Navbar() {
  ...
  const welcomeResponse = session?.user?.id ? await getWelcomeResponse(session.user.id) : null;
    ...
    <header className="sticky top-0 z-30 border-b border-slate-200/90 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <MobileNav signedIn={Boolean(session)} isAdmin={session?.user.role === "admin"} />
        </div>
      </div>
