...
export async function Navbar() {
  ...
  const [identity, community] = await Promise.all([
    session?.user?.id ? getIdentity(session.user.id) : Promise.resolve(null),
    getCommunityStats(),
  ]);
  ...
  return <header className="sticky top-0 z-30 border-b border-slate-200/90 bg-white/90 backdrop-blur-xl">
    <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6">
      <Link href="/" className="group flex shrink-0 items-center gap-2.5 text-lg font-black tracking-[-.04em] text-[#10151f]">
      ...
      <div className="hidden items-center gap-1.5 2xl:flex" aria-label="Vennet community totals">
        <span className="rounded-lg bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">{community.total} members</span>
        <span className="rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-black text-amber-800">{community.verified} verified</span>
        <span className="rounded-lg bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-800">{community.pro} pro</span>
      </div>
