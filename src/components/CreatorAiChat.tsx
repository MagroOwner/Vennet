...
export function CreatorAiChat({ creatorName }: { creatorName: string }) {
  ...
  async function send(message = draft) {
    ...
    if (!clean || busy || remaining === 0) return;
    ...
    try {
      ...
      const payload = await response.json() as { text?: string; error?: string; remaining?: number };
  ...
  return <section className="mt-7 grid gap-6 lg:grid-cols-[.67fr_1.33fr]">
    ...
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5"><div className="flex items-center justify-between border-b border-slate-100 px-6 py-4"><div><p className="font-black text-slate-950">Vennet AI</p><p className="mt-0.5 text-xs font-semibold text-emerald-700">Pro creator assistant</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">Pro</span></div>
      ...
      <form onSubmit={submit} className="border-t border-slate-100 bg-white p-4 sm:p-5"><label htmlFor="ai-message" className="sr-only">Ask Vennet AI</label><textarea id="ai-message" value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={4000} rows={3} placeholder={remaining === 0 ? "Your weekly Vennet AI allowance resets next week." : "Ask Vennet AI for help with your next listing…"} disabled={remaining === 0} className="w-full resize-none rounded-xl border border-slate-300 bg-white p-3 text-sm leading-6 text-slate-950 placeholder:text-slate-500 focus:border-emerald-500 focus:outline-none disabled:bg-slate-100" /><div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-slate-500">AI can make mistakes—review before publishing.</p><button disabled={busy || !draft.trim() || remaining === 0} className="rounded-xl bg-emerald-300 px-5 py-2.5 text-sm font-black text-slate-950 transition hover:-translate-y-0.5 hover:bg-emerald-200 disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Thinking…" : remaining === 0 ? "Weekly limit reached" : "Send →"}</button></div></form>
    </div>
  </section>;
}
