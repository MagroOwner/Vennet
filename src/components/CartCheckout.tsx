...
export function CartCheckout({ listings }: { listings: CartOffer[] }) {
  ...
  async function checkout() { if (!termsAccepted) { setError("Please agree to the Terms of Service before continuing."); return; } setBusy(true); setError(""); const result = await purchaseCart({ termsAccepted: true }); if (result.ok) window.location.assign(result.url); else { setError(result.error); setBusy(false); } }
