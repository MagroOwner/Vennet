...
export default async function HomePage() {
  ...
  return <div className="space-y-14 pb-10 sm:space-y-20">
    <section className="relative overflow-hidden rounded-[2rem] bg-[#10151f] px-6 py-12 text-white shadow-[0_24px_60px_rgb(16_21_31/.18)] sm:px-10 lg:px-14 lg:py-16">
      ...
      <div className="relative grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
        <div className="max-w-2xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3"><Link href="/marketplace" className="button-primary">Browse marketplace <span className="ml-2">→</span></Link><Link href="/dashboard/seller" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/20 bg-white/[.06] px-5 py-3 font-bold text-white transition hover:-translate-y-0.5 hover:bg-white/[.12]">Start selling</Link></div>
    ...
    <section>
      ...
      {featured.length ? <div className="mt-7 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{featured.map((listing, index) => <div key={listing.id} className="animate-rise" style={{ animationDelay: Math.min(index * 65, 260) + "ms" }}><ListingCard listing={listing} saved={false} verifiedSeller={trustByListing[listing.id]?.sellerVerified} rating={trustByListing[listing.id]?.averageRating ? { average: trustByListing[listing.id].averageRating as number, count: trustByListing[listing.id].reviewCount } : null} /></div>)}</div> : <div className="surface-card mt-7 p-10 text-center"><p className="text-lg font-black text-[#10151f]">The first great offer belongs here.</p><p className="mt-2 text-sm text-slate-600">Create a clear listing with a preview and delivery details.</p><Link href="/dashboard/seller" className="button-primary mt-5">Create a listing</Link></div>}
    </section>

    <section className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
      <div className="relative overflow-hidden rounded-[2rem] bg-[#f7e4bd] p-8 shadow-[0_18px_38px_rgb(124_88_21/.10)] sm:p-10"><div className="absolute -right-14 -top-14 h-56 w-56 rotate-12 rounded-[2.5rem] border-[16px] border-white/35" /><div className="relative max-w-md"><p className="text-[11px] font-black uppercase tracking-[.18em] text-amber-900">For sellers</p><h2 className="mt-3 text-4xl font-black leading-tight tracking-[-.04em] text-[#10151f]">Your work deserves a storefront that feels finished.</h2><p className="mt-4 leading-7 text-amber-950/75">Publish digital offers, explain exactly what buyers receive, and keep your sales in one clear workspace.</p><Link href="/dashboard/seller" className="mt-7 inline-flex rounded-xl bg-[#10151f] px-5 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5">Create a listing →</Link></div></div>
