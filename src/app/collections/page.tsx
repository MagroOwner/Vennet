...
export default function CollectionsPage() {
  return <main className="space-y-8 pb-12">
    ...
    <section><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-700">Browse by goal</p><h2 className="mt-1 text-2xl font-black text-slate-950">Categories for every kind of creator</h2></div><p className="text-sm text-slate-600">New work appears as creators publish.</p></div><div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{COLLECTIONS.map((collection, index) => <Link key={collection.slug} href={"/collections/" + collection.slug} className="group animate-rise relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-900/5 transition duration-300 hover:-translate-y-1 hover:shadow-xl" style={{ animationDelay: Math.min(index * 60, 420) + "ms" }}><span className={"grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br " + collection.accent + " text-slate-800 shadow-sm"}><CollectionIcon collection={collection} /></span><h2 className="mt-7 text-xl font-black tracking-tight text-slate-950">{collection.name}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{collection.description}</p><span className="mt-5 inline-flex text-sm font-black text-emerald-800 transition group-hover:translate-x-1">Explore category →</span></Link>)}</div></section>
