...
export default async function MarketplacePage({ searchParams }: { searchParams: { category?: string; collection?: string } }) {
  ...
  return <div className="space-y-8">
    ...
    <section>{listings.length === 0 ? <div className="console-panel p-12 text-center"><p className="text-lg font-bold">Be the first to share something great.</p><p className="mt-2 text-slate-400">List a digital product or service in minutes.</p><Link href="/dashboard/seller" className="button-primary mt-6">Create an offer</Link></div> : <MarketplaceGrid listings={listings} savedListingIds={savedListingIds} initialCollection={collection} heading={collection ? getCollection(collection)?.name + " offers" : category ? "Filtered offers" : "Explore offers"} />}</section>
  </div>;
}
