...
export default async function SellerDashboardPage() {
  ...
  return <div className="console-page">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-semibold text-emerald-300">Seller tools</p><h1 className="mt-1 text-3xl font-black">Your seller dashboard</h1><p className="mt-2 text-slate-400">Create digital listings, review sales, and receive payouts through Stripe.</p></div><Link href="/marketplace/new" className="button-primary">Create a listing</Link></div>
    {!stripeAccount?.chargesEnabled && <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-400/25 bg-amber-400/10 p-5 text-amber-100"><div><p className="font-semibold">Connect Stripe to receive payouts</p><p className="mt-1 text-sm text-amber-100/75">Complete Stripe onboarding before accepting sales. Vennet keeps a 5% platform fee from each sale.</p></div><Link href="/stripe/onboarding" className="button-secondary border-amber-300/30">Connect Stripe</Link></div>}
